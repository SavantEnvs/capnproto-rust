#!/usr/bin/env bash
#
# mayhem/build.sh — build capnproto-rust's three upstream libFuzzer fuzz targets
# (capnp/fuzz/fuzzers/*.rs) as sanitized binaries (OSS-Fuzz Rust path: cargo-fuzz
# + ASan via RUSTFLAGS), build the KAT oracle probe, and precompile the project's
...
#   - This FIRST build (online) populates the cargo registry AND git-checkout
#     cache under $CARGO_HOME (the fuzz crate's libfuzzer-sys is a git dep).
#   - The lockfiles this first build generates (root Cargo.lock,
#     capnp/fuzz/Cargo.lock, mayhem/kat/Cargo.lock) persist in the image layer,
#     so the offline re-run resolves the exact same tree from the cache. The
#     rlenv runtime exports CARGO_NET_OFFLINE=true for the re-run, so we do NOT
#     hard-code `--offline` here (it would break this first, online build).
#
# capnp/fuzz IS ITS OWN WORKSPACE (its Cargo.toml carries [workspace] members =
# ["."]), so `cargo fuzz build` writes binaries under capnp/fuzz/target/<triple>/
# release/ — NOT the root target/ (workspace-membership rule).
#
# capnp/fuzz/build.rs runs capnpc::CompilerCommand, which shells out to the C++
# `capnp` schema tool — apt-installed by mayhem/Dockerfile (baked layer).
set -euo pipefail
...
# project code) BEFORE the first link. Idempotent: re-running --strip-debug on an
# already-stripped archive is a no-op, so the offline PATCH re-run stays clean.
if [ -n "${RUST_SAN}" ]; then
...
# Upstream's own fuzz crate (its own workspace — see header note).
FUZZ_DIR="capnp/fuzz"
TRIPLE="x86_64-unknown-linux-gnu"

# The three upstream targets, declared as [[bin]] entries pointing at fuzzers/*.rs
# (older cargo-fuzz layout; discover from the manifest's [[bin]] names).
FUZZ_TARGETS=(canonicalize serialize_packed_read_no_alloc test_all_types)
...
for t in "${FUZZ_TARGETS[@]}"; do
  ...
  # own-workspace fuzz crate → binary lands under the fuzz crate's target dir.
...
# A small dynamically-linked binary that drives capnp's PUBLIC serialization API
# on fixed inputs and prints exact known-answer values; mayhem/test.sh greps them.
# Built CLEAN (no sanitizer, normal flags) so it is an honest oracle build. Its
# only deps are the in-tree capnp/capnpc path crates — no registry access at all.
...
# so mayhem/test.sh only RUNS it. Scoped to the runtime library, the code
# generator, and the generated-code round-trip suite (capnpc-test) — the same
# packages upstream's own CI exercises — dodging the heavy rpc/example members.
echo "=== precompiling test suite (cargo test --no-run -p capnp -p capnpc -p capnpc-test) ==="
env -u RUSTFLAGS -u CFLAGS -u CXXFLAGS \
  cargo +"${RUST_CHANNEL}" test --no-run -p capnp -p capnpc -p capnpc-test
